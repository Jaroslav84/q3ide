...
void q3ide_overview_detach_all(void)
{
	...
	Q3IDE_SetHudMsg("OVERVIEW off", 1000);
}
...
static void overview_show(void)
{
	...
	if (!q3ide_wm.cap) {
		...
		Q3IDE_SetHudMsg("OVERVIEW: no capture dylib", 2000);
		return;
	}
	if (g_f3_active)
		q3ide_focus3_hide();
	...
	pos[0] = eye[0] + fwd[0] * Q3IDE_OVERVIEW_DIST;
	pos[1] = eye[1] + fwd[1] * Q3IDE_OVERVIEW_DIST;
	pos[2] = eye[2];
	norm[0] = -fwd[0];
	norm[1] = -fwd[1];
	norm[2] = 0.0f;
