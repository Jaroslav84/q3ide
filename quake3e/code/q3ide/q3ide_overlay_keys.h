...
static void ovl_emit_str_sm(float right, float up, const char *s, byte r, byte g, byte b)
{
...
static const char *q3ide_label(int keynum)
{
    ...
    if (keynum == ';')
        return "Pause";
    if (strstr(b, "q3ide_use_key"))
        return "Focus";
    if (strstr(b, "q3ide_lock"))
        return "Laser";
    if (strstr(b, "q3ide_escape"))
        return "Exit";
    if (strstr(b, "q3ide cycle"))
        return "Cycle";
