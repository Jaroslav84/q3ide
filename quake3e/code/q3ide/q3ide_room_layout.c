...
/* Would adding a window of width `extra_w` to a wall that already has `cnt`
 * windows (widths in ws[]) require scaling below MIN_TV_H? */
static qboolean wall_accepts_extra(const float *ws, int cnt, float extra_w, float usable, float tv_h)
{
	/* cnt+1 windows → cnt gaps between them */
	float gaps    = (float) cnt * Q3IDE_LAYOUT_GAP;
	float total_w = gaps + extra_w;
	float scale;
	int j;
	for (j = 0; j < cnt; j++)
		total_w += ws[j];
	if (total_w <= usable)
		return qtrue;
	if (total_w - gaps <= 0.0f)
		return qfalse;
	scale = (usable - gaps) / (total_w - gaps);
	return (scale * tv_h >= Q3IDE_MIN_TV_H) ? qtrue : qfalse;
}
...
int q3ide_room_layout(const q3ide_room_t *room, unsigned int *ids, float *aspects, const int *is_display, int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		for (wi = 0; wi < sorted_room.n; wi++) {
			...
			if (assigned_cnt[wi] >= Q3IDE_MAX_WIN)
				continue;
			ww = ideal_tv_w(wall_tv_h[wi], asp);
			/* Check without writing to assigned_ws */
			if (!wall_accepts_extra(assigned_ws[wi], assigned_cnt[wi], ww,
			                        wall_usable[wi], wall_tv_h[wi]))
				continue;
	...
	for (wi = 0; wi < sorted_room.n; wi++) {
		...
		float total_w, scale, center_z, x_cursor;
		int j;
		...
		/* Scale to fit if needed (should still be above MIN_TV_H by construction) */
		{
			float gaps = (float) (cnt - 1) * Q3IDE_LAYOUT_GAP;
			total_w = gaps;
			for (j = 0; j < cnt; j++)
				total_w += tv_ws[j];
			if (total_w > usable && total_w > 0.0f) {
				scale = (usable - gaps) / (total_w - gaps);
				if (scale < 0.01f)
					scale = 0.01f;
				for (j = 0; j < cnt; j++) {
					tv_ws[j] *= scale;
					tv_hs[j] *= scale;
					/* clamp to MIN_TV_H — this path should not happen but be safe */
					if (tv_hs[j] < Q3IDE_MIN_TV_H) {
						tv_hs[j] = Q3IDE_MIN_TV_H;
						tv_ws[j] = tv_hs[j] * ((aspects && aspects[assigned[wi][j]] > 0.0f)
						                           ? aspects[assigned[wi][j]]
						                           : 16.0f / 9.0f);
					}
				}
			}
		...
		/* Recompute total_w, center the row */
		...
		for (j = 0; j < cnt; j++) {
			...
			if (clear < Q3IDE_WALL_OFFSET) clear = Q3IDE_WALL_OFFSET;
			clear += (float) j * 0.5f;
			pos[0] = wall_surf[0] + wall->normal[0] * clear;
			pos[1] = wall_surf[1] + wall->normal[1] * clear;
			pos[2] = center_z;
			q3ide_queue_push(ids[ii], pos, wall->normal, tv_ws[j], tv_hs[j],
			                 is_display && is_display[ii]);
