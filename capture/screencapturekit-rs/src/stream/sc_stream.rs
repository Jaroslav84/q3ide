...
// Maps stream pointer -> list of handler IDs registered on that stream.
// This allows sample_handler to route frames only to the correct stream's handlers.
static STREAM_HANDLER_MAP: Mutex<Option<HashMap<usize, Vec<usize>>>> = Mutex::new(None);
...
// C callback that retrieves handler from registry, routed by stream pointer
extern "C" fn sample_handler(
    ...
    // Look up which handler IDs belong to this specific stream
    let stream_key = stream as usize;
    let handler_ids: Vec<usize> = {
        let map = STREAM_HANDLER_MAP.lock().unwrap();
        match map.as_ref().and_then(|m| m.get(&stream_key)) {
            Some(ids) => ids.clone(),
            None => {
                // No handlers for this stream - release the buffer
                unsafe { crate::cm::ffi::cm_sample_buffer_release(sample_buffer.cast_mut()) };
                return;
            }
        }
    ...
    if let Some(handlers) = registry.as_ref() {
        // Collect only the handlers that actually exist in the registry
        let valid_entries: Vec<&HandlerEntry> = handler_ids
            .iter()
            .filter_map(|id| handlers.get(id))
            .collect();
        ...
        // Call only the handlers registered on THIS stream
        for (idx, entry) in valid_entries.into_iter().enumerate() {
            ...
            // For all handlers except the last, retain the buffer
            if idx < count - 1 {
...
impl SCStream {
    ...
    pub fn add_output_handler_with_queue(
        ...
        if ok {
            ...
            // Register this handler ID under this stream's pointer
            // so sample_handler routes frames only to this stream's handlers
            {
                let mut map = STREAM_HANDLER_MAP.lock().unwrap();
                if map.is_none() {
                    *map = Some(HashMap::new());
                }
                map.as_mut()
                    .unwrap()
                    .entry(self.ptr as usize)
                    .or_default()
                    .push(handler_id);
            }
    ...
    pub fn remove_output_handler(&mut self, id: usize, of_type: SCStreamOutputType) -> bool {
        ...
        // Remove from stream→handler map
        {
            let mut map = STREAM_HANDLER_MAP.lock().unwrap();
            if let Some(ref mut m) = *map {
                if let Some(ids) = m.get_mut(&(self.ptr as usize)) {
                    ids.retain(|&hid| hid != id);
                    if ids.is_empty() {
                        m.remove(&(self.ptr as usize));
                    }
                }
            }
        }
...
impl Drop for SCStream {
    fn drop(&mut self) {
        // Clean up stream→handler map for this stream
        if !self.ptr.is_null() {
            let mut map = STREAM_HANDLER_MAP.lock().unwrap();
            if let Some(ref mut m) = *map {
                m.remove(&(self.ptr as usize));
...
impl Clone for SCStream {
    ...
    fn clone(&self) -> Self {
        ...
        let new_ptr = unsafe { crate::ffi::sc_stream_retain(self.ptr) };

        // The cloned stream shares the same underlying Swift SCStream (same ptr),
        // so the STREAM_HANDLER_MAP entry is already correct — both the original
        // and clone use the same ptr, which maps to the same handler IDs.
